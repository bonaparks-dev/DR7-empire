                            ...
                            {extendedProfile.tipo_cliente === 'persona_fisica' && (
                                <div className="border-t border-gray-800 pt-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div><label className="text-sm text-gray-400">Codice Fiscale</label><div className="flex gap-2 mt-1"><input type="text" value={formData.codiceFiscale} onChange={handleChange} name="codiceFiscale" className="flex-1 bg-gray-800 border-gray-700 rounded-md text-white p-2 font-mono uppercase" maxLength={16} /><CalcolaCFButton className="px-3 py-2 bg-amber-600 hover:bg-amber-700 text-white text-xs font-medium rounded-md whitespace-nowrap transition-colors" config={{ getCognome: () => formData.lastName, getNome: () => formData.firstName, getDataNascita: () => formData.dataNascita, getSesso: () => formData.sesso, getLuogoNascita: () => formData.cittaNascita, getCodiceFiscale: () => formData.codiceFiscale, setCodiceFiscale: (v) => setFormData(p => ({ ...p, codiceFiscale: v })), setSesso: (v) => setFormData(p => ({ ...p, sesso: v })), setDataNascita: (v) => setFormData(p => ({ ...p, dataNascita: v })), setLuogoNascita: (v) => setFormData(p => ({ ...p, cittaNascita: v })), setProvinciaNascita: (v) => setFormData(p => ({ ...p, provinciaNascita: v })), }} /></div></div>
