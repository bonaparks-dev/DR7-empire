    ...
    >
      <div className="container mx-auto px-4 sm:px-6 py-24 sm:py-32">
        <div className="max-w-4xl mx-auto">
          <div className="mb-8">
            <Link to="/lottery-rules" className="text-white hover:text-yellow-400 transition-colors">
              &larr; {t({ it: "Torna al regolamento", en: "Back to Rules" })}
            </Link>
          </div>
          ...
          <div className="space-y-8">
            {rules.sections.map((section, index) => (
              <section key={index}>
                <h2 className="text-2xl font-semibold font-exo2 mb-4 border-b border-gray-700 pb-2">
                  {t(section.title)}
                </h2>
                <div className="prose prose-invert max-w-none text-gray-300" dangerouslySetInnerHTML={{ __html: t(section.content).replace(/•/g, '<br/>•').replace(/\n/g, '<br />') }} />
              </section>
            ))}
          </div>
        </div>
