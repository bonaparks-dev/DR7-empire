...
const RentalPage: React.FC<RentalPageProps> = ({ categoryId }) => {
  ...
  // Dynamic pricing from revenue config (coefficients: occupation, advance, duration, season)
  const [dynamicPrices, setDynamicPrices] = useState<Record<string, { dailyRate: number; total: number }>>({});

  useEffect(() => {
    if (!prePickup || !preReturn || !vehicleCategory) return;
    // Fetch dynamic prices for all vehicles in parallel
    const fetchPrices = async (vehicles: RentalItem[]) => {
      const results: Record<string, { dailyRate: number; total: number }> = {};
      await Promise.all(
        vehicles.map(async (v) => {
          try {
            const vehicleId = v.id.replace('car-', '');
            const res = await fetch('/.netlify/functions/calculate-dynamic-price', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ vehicle_id: vehicleId, pickup_date: prePickup, dropoff_date: preReturn }),
            });
            if (res.ok) {
              const data = await res.json();
              if (data.enabled && data.finalDailyRateEur) {
                results[v.id] = { dailyRate: data.finalDailyRateEur, total: data.finalTotalEur };
              }
            }
          } catch (e) { /* fallback to simple calculation */ }
        })
      );
      setDynamicPrices(results);
    };
    if (categoryData.length > 0) fetchPrices(categoryData);
  }, [prePickup, preReturn, categoryData.length, vehicleCategory]);
            ...
            >
              {categoryData.map(item => {
                ...
                const marketingTooltip = categoryId === 'urban-cars'
                  ...
                  : undefined;
                // Use dynamic price from revenue config if available, else fallback to simple calc
                const dynPrice = dynamicPrices[item.id];
                const itemTotalPrice = dynPrice ? dynPrice.total
                  : preDays > 0 ? Math.round((item.pricePerDay?.eur || item.priceResidentDaily || 0) * preDays)
                  : undefined;
