        ...
        {mainTab === 'lavaggio' && lavaggioCategory === 'wash' ? (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {/* ABSOLUTE DETAIL — preventivo only */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.2 }}
                transition={{ duration: 0.5, ease: 'easeOut' }}
                className="md:col-span-2 bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-lg overflow-hidden group transition-all duration-300 hover:border-white/50 hover:shadow-2xl hover:shadow-white/10"
              >
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="md:w-1/2 p-6 md:p-8 flex flex-col justify-center">
                    <h3 className="text-2xl md:text-3xl font-bold text-white mb-2">PRIME ABSOLUTE DETAIL</h3>
                    <div className="flex items-center gap-2 mb-4">
                      {detectedCategory ? (
                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                          ...
                          {detectedCategory === 'urban' ? 'URBAN' : 'MAXI'}
                        </span>
                      ) : (
                        <span className="px-3 py-1 rounded-full text-xs font-bold bg-amber-600/20 text-amber-400 border border-amber-600/40">URBAN & MAXI</span>
                      )}
                      <span className="text-gray-400 text-sm">360–480 min</span>
                    </div>
                    <p className="text-white font-semibold mb-3">
                      {lang === 'it' ? 'Tariffa solo su preventivo' : 'Quote-based pricing only'}
                    </p>
                    <ul className="text-gray-300 text-sm space-y-2 mb-6">
                      <li>• {lang === 'it' ? 'Smontaggio completo delle sedute e accesso alle strutture interne dell\'abitacolo' : 'Complete seat disassembly and access to interior cabin structures'}</li>
                      <li>• {lang === 'it' ? 'Trattamento macchie' : 'Stain treatment'}</li>
                      <li>• {lang === 'it' ? 'Decontaminazione profonda di guide, moquette, pannellature in zone nascoste e intercapedini' : 'Deep decontamination of rails, carpets, panels in hidden areas'}</li>
                      <li>• {lang === 'it' ? 'Intervento mirato su sporco stratificato, residui organici, batteri e contaminazioni invisibili' : 'Targeted treatment of layered dirt, organic residues, bacteria and invisible contamination'}</li>
                    </ul>
                    <p className="text-gray-400 text-xs italic mb-6">
                      {lang === 'it' ? 'Non è un lavaggio. È un reset tecnico totale.' : 'This is not a wash. It\'s a total technical reset.'}
                    </p>
                    {targaResult && detectedCategory ? (
                      <a
                        ...
                        className="inline-flex items-center justify-center gap-2 bg-green-600 text-white px-8 py-3 rounded-full font-bold text-sm hover:bg-green-500 transition-all duration-300"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center justify-center gap-2 bg-gray-700 text-gray-300 px-8 py-3 rounded-full font-bold text-sm hover:bg-gray-600 transition-all duration-300"
                      >
                        {lang === 'it' ? 'Cerca la targa per richiedere il preventivo' : 'Search plate to request quote'}
                      </button>
                    )}
                  </div>
                </div>
