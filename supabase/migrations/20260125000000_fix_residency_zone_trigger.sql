-- Fix sync_auth_user_to_customers trigger to handle residency_zone field
-- This resolves the 400 error during sign-up when residency_zone is provided
...
BEGIN
  ...
  IF NEW.email IS NOT NULL THEN
    ...
    IF existing_id IS NULL THEN
      -- Insert new customer with ALL available fields including residency_zone
      ...
      RAISE NOTICE 'Auto-created customer for user: % (email: %, CF: %, residency: %)', 
        NEW.id, NEW.email, codice_fiscale_value, residency_zone_value;
    ELSE
      ...
      UPDATE customers_extended SET
        ...
        residency_zone = COALESCE(residency_zone_value, residency_zone),
...
'Automatically creates/updates customers_extended record when a new auth user is created. Extracts ALL available fields from raw_user_meta_data including CF, address, birth info, license metadata, company fields, PA fields, and residency_zone.';
