-- Fix MAX(id) error with UUID columns
-- PostgreSQL doesn't support MAX() on UUID without explicit casting
...
BEGIN
  ...
  SELECT
    COUNT(*),
    (ARRAY_AGG(id ORDER BY created_at DESC))[1], -- Get most recent conflict ID
...
-- Add helpful comment
COMMENT ON FUNCTION check_unified_vehicle_availability IS 'Fixed version: uses ARRAY_AGG instead of MAX for UUID columns';
COMMENT ON FUNCTION check_unified_carwash_availability IS 'Fixed version: uses ARRAY_AGG instead of MAX for UUID columns';
