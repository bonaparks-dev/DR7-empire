...
const CarBookingWizard: React.FC<CarBookingWizardProps> = ({ item, categoryContext, onBookingComplete, onClose }) => {
  ...
  const [formData, setFormData] = useState(() => {
    ...
    return {
      ...
      extras: [] as string[],
      kmPackageType: 'unlimited' as 'none' | 'unlimited',
  ...
  const [dynamicPricing, setDynamicPricing] = useState<{
    ...
    rentalDays?: number
    breakdown?: { coeff: number; label?: string }[]
  } | null>(null)
  ...
  useEffect(() => {
    ...
    if (vType === 'SUPERCAR') {
      // Supercars default to standard pricing from Centralina
      setFormData(prev => ({
        ...prev,
        kmPackageType: 'none'
      }));
  ...
  */
  ...
  const {
    ...
    extraDayApplied,
    listSubtotal, hasDynamicDiscount, dynamicDiscountPct, combinedCoeff
  } = useMemo(() => {
    const zero = {
    ...
    const combinedCoeff = hasDynamicCoeffs
      ? (dynamicPricing!.breakdown!.reduce((acc: number, b: { coeff: number }) => acc * b.coeff, 1))
      : 1;
    const hasDynamicDiscount = hasDynamicCoeffs && Math.abs(combinedCoeff - 1) > 0.001;
    const dynamicDiscountPct = hasDynamicDiscount ? Math.round((1 - combinedCoeff) * 100) : 0;
    if (hasDynamicCoeffs) {
            ...
            <section className="border-t border-gray-700 pt-6">
              ...
              <div className="p-6 bg-gray-800/50 rounded-lg border border-gray-700 space-y-6 text-sm">
                ...
                <div>
                  ...
                  <p>Pacchetto km: {isUrbanOrCorporate ? 'Inclusi' : (formData.kmPackageType === 'unlimited' || includedKm >= 9999) ? 'ILLIMITATI' : `${includedKm} km`}</p>
                </div>
            ...
            {showPrepaidWarning && (
              <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                <div className="bg-[#1a1a2e] rounded-2xl max-w-md w-full p-6 border border-red-500/30 shadow-2xl">
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => { setShowPrepaidWarning(false); setFormData(prev => ({ ...prev, depositOption: '' })); }}
                      className="flex-1 py-3 px-4 rounded-xl border border-gray-600 text-gray-300 hover:bg-gray-800 transition-colors text-sm font-medium"
                    >
                      Annulla
                    </button>
                    <button
                      ...
                      className="flex-1 py-3 px-4 rounded-xl bg-white hover:bg-gray-200 text-black font-bold transition-colors text-sm"
                    >
                      Accetto e Procedo
                    </button>
                  </div>
                ...
                <aside className="lg:col-span-1 lg:sticky lg:top-32 self-start mb-8 lg:mb-0">
                  <div className="bg-gray-900/50 p-6 rounded-lg border border-gray-800">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between"><span className="text-gray-400">Pacchetto chilometrici</span><span className="text-white font-medium">{kmPackageCost === 0 ? 'Incluso' : formatPrice(kmPackageCost)}</span></div>
