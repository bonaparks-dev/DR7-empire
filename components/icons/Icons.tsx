...
export const TiktokIcon: React.FC<{ className?: string }> = ({ className = "w-6 h-6" }) => ( <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={className}><path d="M12.525.02c1.31-.02 2.61-.01 3.91.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.38 1.92-3.54 2.99-5.8 3.37-1.85.31-3.66.24-5.46-.22-2.43-.63-4.32-2.1-5.52-4.32-1.2-2.24-1.48-4.83-1.03-7.31.42-2.33 1.83-4.29 3.8-5.57 2.82-1.85 6.36-2.11 9.49-1.03v4.03c-1.31.02-2.61.01-3.91-.02-1.53-.08-3.09-.63-4.17-1.75-1.11-1.12-1.62-2.7-1.79-4.24V.02z" /></svg> );
export const WhatsAppIcon: React.FC<{ className?: string }> = ({ className = "w-6 h-6" }) => ( <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={className}><path d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91 0 1.75.46 3.35 1.25 4.76L2 22l5.45-1.44c1.35.71 2.86 1.12 4.45 1.12h.14c5.46 0 9.91-4.45 9.91-9.91s-4.45-9.91-9.91-9.91zM17.31 15.26c-.28-.14-1.68-.83-1.94-.92-.26-.1-.45-.14-.64.14-.18.28-.73.92-.9 1.11-.16.18-.32.21-.6.07-.28-.14-1.18-.43-2.25-1.39-1.07-1.07-1.59-2.02-1.77-2.36-.18-.34-.01-.52.12-.66.12-.12.26-.32.4-.48.12-.16.16-.28.25-.47.08-.18.04-.36 0-.5s-.64-1.54-.88-2.1c-.24-.56-.48-.48-.64-.48-.16 0-.34-.03-.53-.03s-.45.07-.68.34c-.24.28-.88.86-.88 2.1s.9 2.44 1.03 2.62c.12.18 1.77 2.71 4.29 3.79 2.52 1.08 2.52.72 2.97.68.45-.04 1.68-.68 1.92-1.34.24-.65.24-1.21.16-1.34c-.06-.13-.24-.21-.52-.35z" /></svg> );
